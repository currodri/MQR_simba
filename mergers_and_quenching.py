...
def quench_merger_withreju():
    ...
    ax = fig.add_subplot(1,1,1)
    sc = ax.scatter(quench_t, merger_t, c = np.log10(rejuvenation_t-merger_t), cmap='winter')
    ax.plot([quench_t.min()*0.8, quench_t.max()*1.2], [merger_t.min()*0.8, merger_t.max()*1.2], 'k--')
