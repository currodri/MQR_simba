...
def SFR_Evolution2(mergers, msq_galaxies, n_bins):
    ...
    for i in range(0, n_bins-1):
        ...
        ssfr_m_ave[i], ssfr_m_error[i] = plotmedian(mergers_m,ssfr_m, pos=pos_m, boxsize=d['boxsize_in_kpccm'],bin_choosen=[ssfr_m.min()*0.9,ssfr_m.max()*1.1])
        ssfr_nm_ave[i], ssfr_nm_error[i] = plotmedian(msq_m,ssfr_nm, pos=pos_nm, boxsize=d['boxsize_in_kpccm'],bin_choosen=[ssfr_nm.min()*0.9,ssfr_nm.max()*1.1])
