...
def distanceMSQ_2(mergers, sf_galaxies, nbins):
    ...
    for i in range(0, len(ylabels)):
        ...
        for m in range(0, len(titles)):
            ...
            print('Real')
            print(bins)
            mer_cen, mer_median, mer_var = plotmedian2(mer_m,mer,pos=mer_pos,boxsize=d['boxsize_in_kpccm'], bins=bins)
            #mer_median, mer_var = plotmedian(mer_m,mer,pos=mer_pos,boxsize=d['boxsize_in_kpccm'],bin_choosen=bins)
            msq_m = np.asarray(msq_m)
            msq = np.asarray(msq)
            msq_cen, msq_median, msq_var = plotmedian2(msq_m,msq,pos=msq_pos,boxsize=d['boxsize_in_kpccm'], bins=bins)
