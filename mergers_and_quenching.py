...
def quench_merger_scatter():
    ...
    cb = fig.colorbar(sc, ax=ax, orientation='horizontal', fontsize=16)
    cb.set_label(label=r'$\log(T_q/t_H)$', fontsize=14)
    fig.tight_layout()
...
def merger_reju_scatter():
    ...
    ax.set_ylabel(r'$T_m $(Gyr)', fontsize=16)
    cb = fig.colorbar(sc, ax=ax, orientation='horizontal', fontsize=16)
    cb.set_label(label=r'$\log(\Delta sSFR)$', fontsize=14)
    fig.tight_layout()
