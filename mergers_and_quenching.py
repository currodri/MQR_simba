...
def mqr_relation():
    ...
    ax.step(bin_cent, hist, where='mid', 'k', label='Total quenchings')
    median = np.median(time_diff)
    ax.plot([median, median],[0, hist.max()], 'k:', linewidth=4.0)
    hist, bin_edges = np.histogram(time_diff_s, bins=binis)
    bin_cent = 0.5*(bin_edges[1:]+bin_edges[:-1])
    hist = hist/np.sum(d['sf_galaxies_per_snap'])
    ax.step(bin_cent, hist, where='mid', 'b', label='Slow quenchings')
    median = np.median(time_diff_s)
    ax.plot([median, median],[0, hist.max()], 'b:', linewidth=4.0)
    hist, bin_edges = np.histogram(time_diff_f, bins=binis)
    bin_cent = 0.5*(bin_edges[1:]+bin_edges[:-1])
    hist = hist/np.sum(d['sf_galaxies_per_snap'])
    ax.step(bin_cent, hist, where='mid', 'r', label='Fast quenchings')
    median = np.median(time_diff_f)
    ax.plot([median, median],[0, hist.max()], 'r:', linewidth=4.0)
    ...
    bin_cent = 0.5*(bin_edges[1:]+bin_edges[:-1])
    hist = hist/np.sum(d['sf_galaxies_per_snap'])
    ax.plot([median, median],[0, hist.max()], 'm:', linewidth=4.0)
    ax.step(bin_cent, hist, where='mid', 'm', label='Rejuvenations')
