...
def SFR_Evolution2(mergers, msq_galaxies, n_bins):
    ...
    print(z_bins)
    cent_m, ssfr_m_ave, ssfr_m_error = plotmedian2(red_m,ssfr_m, pos=pos_m, boxsize=d['boxsize_in_kpccm'])
    cent_nm, ssfr_nm_ave, ssfr_nm_error = plotmedian2(red_nm,ssfr_nm, pos=pos_nm, boxsize=d['boxsize_in_kpccm'])
...
def SFR_Evolution_and_Contribution(mergers, msq_galaxies, n_bins):
    ...
    axes[0].errorbar(np.log10(1+cent_m), np.log10(ssfr_m_ave), yerr=np.log10(ssfr_m_ave+ssfr_m_error), linestyle='--', marker='o', label='Mergers star-forming', capsize=2, capthick=2)
    axes[0].errorbar(np.log10(1+cent_nm), np.log10(ssfr_nm_ave), yerr=np.log10(ssfr_nm_ave+ssfr_nm_error), linestyle='--', marker='s', label='Mass-matched sample of non-merger star-forming', capsize=2, capthick=2)
